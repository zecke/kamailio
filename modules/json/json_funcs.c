...
int jsonmod_get(struct sip_msg* msg, char* path_in, char* src_in, char* dst)
{
	str src_s;
	str path_s;
	pv_spec_t *dst_pv;
	...
	if(json_to_val(&dst_val, &freeme, v)<0) goto fail;
...
*/
	...
	if(json_to_val(&result_val, &freeme, result_json)<0) goto fail;
...
int jsonmod_array_size(struct sip_msg* msg, char* path_in, char* src_in, char* dst)
{
	str src_s;
	str path_s;
	pv_spec_t *dst_pv;
