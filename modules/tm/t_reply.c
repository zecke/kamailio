...
#define STATUS_TABLE_SIZE 1000
#define CANCELED_STATUS (STATUS_TABLE_SIZE - 1)

     /* The table containing priorities of status codes */
static int status_priority[STATUS_TABLE_SIZE];
...
void init_status_table(void)
{
	...
	for(i = 0; i < STATUS_TABLE_SIZE; i++) {
		status_priority[i] = i * 10;
	}

	     /* Positive replies get very high priority, they should
	      * be never overriden by anything else
	      */
	for(i = 200; i <= 299; i++) {
		status_priority[i] = (i - 200) * 10;
	}

	     /* 487 generated by incoming CANCEL are more important than any
	      * other incoming negative reply, but not as important as positive
	      * replies
	      */
	status_priority[CANCELED_STATUS] = 991;

	     /* 6xx class of replies gets lower priority than 2xx but
	      * higher priority than anything else
	      */
	for(i = 600; i <= 699; i++) {
		status_priority[i] = (i - 600) + 1000;
	}

	status_priority[401] = 3991; /* 401 Unauthorized */
	status_priority[407] = 3992; /* 407 Proxy Authentication Required */
	status_priority[415] = 3993; /* 415 Unsupported Media Type */
	status_priority[423] = 3994; /* 423 Interval Too Brief */
	status_priority[484] = 3995; /* 484 Address Incomplete */
	status_priority[486] = 3996; /* 486 User Busy */
	status_priority[480] = 3997; /* 480 Temporarily Unavailable */
}


/* select a branch for forwarding; returns:
 * 0 .. (MAX_BRANCHES - 1) ... branch number
 * MAX_BRANCHES            ... localy generated reply (i.e. 487 triggered by CANCEL)
 * -1                      ... error
 * -2                      ... can't decide yet -- incomplete branches present
 */
int t_pick_branch(int inc_branch, int inc_code, struct cell* t, int* res_code)
{
	int min_branch, min_priority, b, best_code;

	min_branch = -1;
	min_priority = INT_MAX;
	best_code = 999;
	
	     /* Test for canceled transaction */
	if (t->flags & T_CANCELED_FLAG) {
		if (status_priority[CANCELED_STATUS] < min_priority) {
			min_branch = MAX_BRANCHES;
			min_priority = status_priority[CANCELED_STATUS];
			best_code = 487;
		}
	}
	...
	for(b = 0; b < t->nr_of_outgoings; b++) {
		...
		if (b == inc_branch) {
			if (status_priority[inc_code] < min_priority) {
				min_branch = b;
				min_priority = status_priority[inc_code];
				best_code = inc_code;
			}
			continue;
		}
		...
		if (t->uac[b].last_received < 200) return -2;
		if (status_priority[t->uac[b].last_received] < min_priority) {
			min_branch = b;
			min_priority = status_priority[t->uac[b].last_received];
			best_code = t->uac[b].last_received;
		}
	} /* find lowest branch */
