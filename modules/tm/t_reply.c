...
{
	int branch_cnt, picked_branch, picked_code, inv_through;

	DBG("tm:t_should_relay_response: T.uas.code=%d, new_code=%d\n", Trans->uas.status, new_code);
	...
	if (Trans->uac[branch].last_received >= 200
		...
		if (Trans->uac[branch].last_received == new_code) {
			DBG("DBG:t_should_relay_response: final reply retransmission\n");
			goto discard;
		}
		...
		if (Trans->uac[branch].last_received == 408 && new_code == 487) {
			DBG("DBG:t_should_relay_response 487 came for a timed-out branch\n");
			goto discard;
		}
